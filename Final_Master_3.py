...
def process_data(df,data_type,target_type):
    ...
    transformation = alb.Compose([alb.RandomResizedCrop(height =IMAGE_SIZE,width =IMAGE_SIZE,scale=(0.7,0.9),p=.4),
...
def model_definition(pretrained=False):
    ...
    if pretrained == True:
        ...
        model.classifier = nn.Sequential(nn.Linear(model.classifier.in_features, IMAGE_SIZE),
                                        nn.Dropout(DROPOUT),
