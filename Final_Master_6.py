...
#Need to install albumentations
import random
...
def process_data(df,data_type,target_type):
    ...
    transformation = alb.Compose([alb.RandomGamma(p=.2),
                                  alb.RandomBrightness(p=.4),
                                  alb.RandomContrast(p=.4),
                                alb.RandomResizedCrop(height =IMAGE_SIZE,width =IMAGE_SIZE,scale=(0.7,0.9),p=.2),
                                  alb.HorizontalFlip(p=0.4),
                                  alb.VerticalFlip(p=0.4),
                                  alb.Resize(IMAGE_SIZE,IMAGE_SIZE)])

    if data_type == 'train':
        data_set = Dataset(list_of_ids, data_type,target_type,transform = transformation)
    else:
        data_set = Dataset(list_of_ids, data_type, target_type, transform= transformation)
...
def model_definition(pretrained=False):
    ...
    if pretrained == True:
        ...
        model.classifier = nn.Sequential(nn.Linear(1536, IMAGE_SIZE),
                                        nn.Dropout(DROPOUT),
                                        nn.Linear(IMAGE_SIZE, OUTPUTS_a),
                                        nn.Sigmoid())
    else:
        model = CNN()
...
def train_and_test(train_ds, test_ds, list_of_metrics, list_of_agg,class_weights, save_on, pretrained = False):
...
class_names = np.unique(Label_A)
class_weights = []
for name in class_names:
    weight = (len(train_df['label']) / (len(train_df[train_df['label'] == name]) * len(class_names)))
...
list_of_metrics = ['acc', 'f1_weighted']
list_of_agg = ['avg']
...
train_and_test(train_ds, val_ds, list_of_metrics, list_of_agg,class_weights, save_on='acc', pretrained=True)